...
export function DashboardLayout(){
 const {role,user,signOut}=useAuth();const navigate=useNavigate();const[mobileOpen,setMobileOpen]=useState(false);const nav=role==="admin"?adminNav:role==="specialist"?specialistNav:customerNav;const onSignOut=async()=>{await signOut();navigate("/")};
 const SidebarFooter=()=> <div className="border-t bg-card/60 p-3"><div className="mb-2 min-w-0 rounded-xl bg-gradient-to-br from-accent/70 to-card px-3 py-2.5 text-xs text-muted-foreground"><div className="truncate font-medium text-foreground">{user?.email}</div><div className="mt-1 flex items-center gap-1 capitalize"><ShieldCheck className="h-3 w-3 shrink-0"/>{role}</div></div><Button variant="ghost" size="sm" className="w-full justify-start rounded-xl" onClick={onSignOut}><LogOut className="mr-2 h-4 w-4"/>Sign out</Button></div>;
