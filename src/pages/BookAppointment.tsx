...
import { CalendarClock, CheckCircle2, Clock3, Loader2, ShieldCheck, Sparkles } from "lucide-react";
...
function nextWorkingDays(from = new Date()) {
  const result: Date[] = [];
  let cursor = startOfDay(from);
  while (result.length < WORKING_DAYS) {
    cursor = addDays(cursor, 1);
    const day = getDay(cursor);
    if (day !== 0 && day !== 6) result.push(cursor);
  }
  return result;
}
